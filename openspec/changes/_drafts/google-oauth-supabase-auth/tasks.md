...
- [ ] 1.4 smoke：建一張測試 profile + 啟 RLS → 未登入 SELECT 應成功、未登入寫入應被拒（驗 RLS 生效，REF-004 坑）
...
- [ ] 2.4 [Next.js 16 相容校準] 確認 middleware 檔名仍為 `middleware.ts`（非 `proxy.ts`），並驗證 `cookies()` 為 async API（`await cookies()`）；若行為與 REF-002 不符以實測結果為準
...
- [ ] 3.4 `src/app/auth/callback/route.ts`（exchangeCodeForSession + 錯誤頁 fallback + `next` 參數白名單驗證：僅接受相對路徑，否則 fallback 至 `/`）
...
- [ ] 6.1 從 profiles 查公開名片（取代 mockPlayers）
- [ ] 6.2 is_tag_visible=false 時遮蔽 battle_tag
