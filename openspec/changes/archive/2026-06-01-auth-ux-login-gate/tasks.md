...
- [x] 0.1 確認 dev server 可啟動（`npm run dev`），auth callback 流程正常（上一個 change 已驗證）
- [x] 0.2 確認 TypeScript 無現有錯誤（`npx tsc --noEmit`）

## 1. 建立 `LoginModal` 元件（D3, REF-006）

- [x] 1.1 建 `src/components/LoginModal.tsx`（`"use client"`）
...
- [x] 1.2 TypeScript 確認無錯誤

## 2. 首頁右上角加登出按鈕（D1）

- [x] 2.1 修改 `src/app/page.tsx` 登入後區塊（line 88-96）
  - 改為 `<div className="flex items-center gap-2">` 包住兩個元素
  - 保留「我的名片」Link（現有樣式不動）
  - 新增「登出」按鈕：ghost 風格（邊框 `/20`、文字 `/70`），加 `LogOut` icon（`lucide-react`）
- [x] 2.2 實作 `handleLogout`：`await supabase.auth.signOut()` + `router.refresh()`（防 RSC cache 不同步）
- [x] 2.3 加入 `import { LogOut } from "lucide-react"` + `import { useRouter } from "next/navigation"` + `const router = useRouter()`
- [x] 2.4 瀏覽器驗證：登入後右上角出現並排按鈕，點登出後切回登入狀態

## 3. `/profile` 頁面未登入 Overlay（D2, 使用 LoginModal）

- [x] 3.1 修改 `src/app/profile/page.tsx`
...
- [x] 3.2 瀏覽器驗證：未登入訪問 `/profile`，overlay 出現；已登入時 overlay 不出現

## 4. OWCard 接 `onLoginRequired`（D4）

- [x] 4.1 修改 `src/components/OWCard.tsx`
...
- [x] 4.2 修改 `src/app/browse/page.tsx`
...
- [x] 4.4 瀏覽器驗證：未登入點 OWCard 複製按鈕，Modal 彈出；點 × 關閉

## 5. 整合測試

- [x] 5.1 登入流程：首頁點「使用 Google 登入」→ Google 授權 → callback → 回首頁右上角顯示「我的名片 + 登出」
- [x] 5.2 登出流程：點「登出」→ 回未登入狀態（「使用 Google 登入」出現）
- [x] 5.3 Profile overlay：未登入訪問 `/profile`，overlay 蓋住頁面；點 Google 登入觸發 OAuth
- [x] 5.4 Browse LoginModal：未登入，點名片複製按鈕，Modal 彈出；點 × 關閉；點 Google 登入觸發 OAuth
- [x] 5.5 TypeScript 全 pass：`npx tsc --noEmit`
