...
| 資料授權 | Postgres RLS：profiles 僅 authenticated-own-row；公開讀取透過 `public_profiles` view（DB 層隱私遮蔽）| REF-004（anon key 不直接開放整表；view 層保證 social_channels 不公開、is_tag_visible=false 的 battle_tag 遮蔽）|
...
-- 無 public SELECT policy：anon key 不能直接讀 profiles 整表
...
-- 公開讀取介面：view 層 DB 遮蔽，anon key 只能透過此 view 讀廣場資料
...
**做：** Google OAuth 登入/登出、`/profile` 登入後讀寫自己的 profile、`/browse` 從 `public_profiles` view 讀公開名片（DB 層隱私遮蔽）、RLS + public_profiles view、middleware token 刷新。
...
| 風險 | 緩解 |
|---|---|
| RLS enable 後漏建 policy → 整表打不開（REF-004 坑） | apply Task 4.4 smoke：anon 查 `profiles` 應被拒/空、查 `public_profiles` 應成功且遮蔽正確、authenticated 寫入應成功 |
