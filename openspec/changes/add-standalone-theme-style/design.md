...
- 「145 處 / 11 檔」：rsx-explorer grep 實測（REF-034）。**Phase 0.2 校準（F-028）**：完整掃描實測全 src ~1,562 處 / 31 檔；其中 ~1,100 處在開發者後台頁（內部工具，排除遷移、列豁免清單）；**實際遷移範圍 = 使用者面向 ~419 處 / 23 檔，去重後 127 種 class 值**。後文「145 處」一律以此校準值取代。
