...
export default function DeveloperConsoleClient({
  ...
  // debounce cleanup on unmount（防 memory leak）
  useEffect(() => {
    return () => {
  ...
  return (
    <div className={`min-h-screen flex flex-col font-sans selection:bg-[#82b7cc] selection:text-slate-900 transition-colors duration-500 ${
      ...
      {/* 背景裝飾（radial-gradient 取代 blur-[150px]，零 GPU compositing 成本）*/}
      {activeApcTool !== "homepage" ? (
        <>
        ...
        <>
          <div className="absolute top-[-10%] left-[-10%] w-[50vw] h-[50vw] pointer-events-none" style={{ background: "radial-gradient(circle, rgba(130,183,204,0.05) 0%, transparent 70%)" }} />
          <div className="absolute bottom-[-10%] right-[-10%] w-[50vw] h-[50vw] pointer-events-none" style={{ background: "radial-gradient(circle, rgba(245,212,107,0.08) 0%, transparent 70%)" }} />
        </>
      )}
