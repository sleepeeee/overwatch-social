...
export default function BrowsePage() {
  ...
  return (
    <div className="relative min-h-screen z-10 selection:bg-auroraMint/30 text-theme-text-strong max-w-7xl mx-auto px-4 sm:px-6 py-5 sm:py-8 space-y-5 sm:space-y-8">
      {/* 全螢幕無縫大氣漸層 */}
      ...
      <section className="space-y-5 sm:space-y-8 animate-fade-in relative z-10">
        ...
        <div className="glass-card p-4 sm:p-6 rounded-xl space-y-3 sm:space-y-4">
          <p className="text-[10px] text-theme-text-muted uppercase tracking-widest font-mono">
            選擇遊戲星系 // Game Directory
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-[auto_1fr] gap-3 sm:gap-4 items-center">
            <div className="flex items-center space-x-2 max-w-md">
              <span className="text-[11px] text-theme-text-muted whitespace-nowrap font-mono">星系:</span>
              <div className="relative w-full">
                ...
                >
                  {gameTabs.map((game) => (
                    <option key={game.id} value={game.id} className="bg-obsidian text-theme-text-soft">
                      {game.optionLabel}
                    </option>
                  ))}
                </select>
            ...
            <p className="text-[11px] text-theme-text-faint leading-relaxed">
