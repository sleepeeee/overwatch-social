...
interface LotusWelcomeWidgetProps {
  previewData?: AnnouncementItem[];
  activeStepOverride?: number;
}

export default function LotusWelcomeWidget({ previewData, activeStepOverride }: LotusWelcomeWidgetProps = {}) {
  const [localActiveStep, setLocalActiveStep] = useState(1);
  const activeStep = activeStepOverride !== undefined ? activeStepOverride : localActiveStep;
    ...
    >
      ...
      >
        {current.custom_icon_url ? (
          <img 
            src={current.custom_icon_url} 
            alt={current.title}
            className="w-20 h-20 object-contain rounded-xl drop-shadow-[0_4px_12px_rgba(130,183,204,0.3)] transform hover:scale-105 active:scale-98 transition-transform duration-500 cursor-pointer" 
          />
        ) : (
      ...
      >
        {announcements.map((s, idx) => {
          ...
          return (
            <button
              key={s.num}
              onClick={() => setLocalActiveStep(stepNum)}
